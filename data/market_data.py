...
def get_calendar(start: str, end: str) -> List[Any]:
    """Fetch Alpaca trading calendar between two dates (YYYY-MM-DD)."""
    try:
        api = _get_api()
        return api.get_calendar(start=start, end=end)
    except Exception as e:
        logger.warning("get_calendar failed: %s", e)
        return []
