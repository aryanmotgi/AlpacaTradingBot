...
from notifications.daily_summary import record_signal, record_scanner_picks, record_trade, record_llm_decision
...
def run_signal_scan() -> None:
    ...
    try:
        ...
        for symbol in _daily_watchlist:
            # Check if we already have a position in this underlying symbol
            already_held = False
            for p in positions:
            ...
            if already_held:
                logger.debug("Already holding %s; skipping new entry", symbol)
                continue
            ...
            try:
                ...
                record_llm_decision(symbol, sig["signal"], llm_result)
...
def main() -> None:
    ...
    # Recurring scan every 30 min from 07:00 to 12:30 PT
    scheduler.add_job(
        run_signal_scan,
        CronTrigger(minute="0,30", hour="7,8,9,10,11,12"),
        id="recurring_scan",
    )
