...
def calculate_position_size(account_value: float, option_price: float) -> int:
    """
    Max contracts to buy so total cost <= MAX_POSITION_PCT of portfolio.
    option_price is per-contract (e.g. premium * 100).
    """
    if account_value <= 0 or option_price <= 0:
        return 0
    
    # 1. Hard Dollar Cap (10% of portfolio)
    max_dollars = account_value * (MAX_POSITION_PCT / 100.0)
    
    # 2. Cost per 1 contract (premium * 100)
    cost_per_contract = option_price * 100
    
    # 3. Calculate quantity (Always round down to stay under budget)
    contracts = int(max_dollars / cost_per_contract)
    
    # Safety Check: If it costs even 1 cent more than max_dollars, reduce qty
    if (contracts * cost_per_contract) > max_dollars:
        contracts -= 1
